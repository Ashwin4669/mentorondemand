...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity httpSecurity) throws Exception {
		...
		httpSecurity.csrf().disable().httpBasic().and().authorizeRequests().antMatchers("/authenticate").permitAll()
				...
				.antMatchers(HttpMethod.OPTIONS, "/**").permitAll().antMatchers("/proposal/{mentorName}/{traineeName}/{techName}").permitAll()
				.antMatchers(HttpMethod.OPTIONS, "/**").permitAll().antMatchers("/proposal/pending/{mentorName}").permitAll()
				.antMatchers(HttpMethod.OPTIONS, "/**").permitAll().antMatchers("/proposal/all/{mentorName}").permitAll()
				.antMatchers(HttpMethod.OPTIONS, "/**").permitAll().antMatchers("/proposal/accept/{trainingId}").permitAll()
				.antMatchers(HttpMethod.OPTIONS, "/**").permitAll().antMatchers("/proposal/incomplete/{traineeId}").permitAll()
